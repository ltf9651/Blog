...
- 规范
  ...
  - 避免在循环内做运算、判断
  - 相同功能模块的表使用统一的表前缀
  - `date()`、`strtotime()`等时间函数默认值大多为`time()`，需要注意和`$_SERVER['REQUEST_TIME']`统一
- 安全
  - 去重、考虑 null
  - 重视代码对性能的影响
  - 项目上线前检查语法错误
...
时间加减计算尽量使用`strtotime`,`new DateTime()`，直接使用数字进行加减在有东夏令时时区切换的地区会有问题
...
- 不推荐使用 MD5，SHA-1，CRC32
- php函数：`password_hash()`,`password_verify()`